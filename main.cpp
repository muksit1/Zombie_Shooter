...
void HandleWeaponKey(int key, WeaponType weapon) {
    if (g_game.keys[key] && !g_game.weaponSwitchHeld[weapon]) {
        SwitchWeapon(weapon);
        g_game.weaponSwitchHeld[weapon] = true;
    }
    if (!g_game.keys[key]) {
        g_game.weaponSwitchHeld[weapon] = false;
    }
}
...
void UpdateGame(float deltaTime) {
    ...
    HandleWeaponKey('1', WEAPON_PISTOL);
    HandleWeaponKey('2', WEAPON_RIFLE);
    HandleWeaponKey('3', WEAPON_BATTLE_RIFLE);
    HandleWeaponKey('4', WEAPON_SHOTGUN);
